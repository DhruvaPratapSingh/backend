...
const generateAccessAndRefreshTokens = async (userId) => {
  try {
    ...
    throw new ApiError(
      500,
      "something went wrong to generating refresh and aceess token"
    );
  }
};
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  const channel = await User.aggregate([
    ...
    // count how many subscribers you have
    {
      $lookup: {
        from: "subscriptions",
        localField: "_id",
    ...
    // count how many channels you subscribed
    {
      $lookup: {
        from: "subscriptions",
        localField: "_id",
    ...
    },
    {
      $addFields: {
        ...
        //    show the btn if subscribed send to frontend
        isSubscribed: {
          $cond: {
  ...
  console.log("channel in user controller", channel);
  if (!channel?.length) {
